...
function draw(dt, clearOnly){
  ...
  CTX.save();
  for (let i=0;i<10;i++){ const y = (i/10)*H; CTX.strokeStyle = 'rgba(0,240,255,0.06)'; CTX.beginPath(); CTX.moveTo(0,y); CTX.lineTo(W,y); CTX.stroke(); }
  CTX.restore();
  if (clearOnly) return;
...
function drawHud(){
  ...
  CTX.fillText(goalText, pad, pad + Math.round(24*UISCALE));
...
function drawPauseSheet(){
  ...
  CTX.font = `700 ${Math.round(22*UISCALE)}px Inter, system-ui, sans-serif`;
  const text = 'Пауза — враги скрыты';
  const m = CTX.measureText(text);
  const cx = W/2 - m.width/2;
  const cy = H/2;
